...
#ifdef CONFIG_MACH_MSM8930_FX3 // AICL for USB
	if (usb_target_ma == 0
		&& the_chip->aicl
#ifdef CONFIG_LGE_PM_VZW_FAST_CHG
...
static int chg_update_charging_state(void *data, int batt_temp, int batt_level)
{
	...
	if (count++ >= 20) {
		pr_info("USBIN:%d, chg_gone:%d, usb:%d, ac:%d, iusb:%d, ibatMax1:%d, ibatMax2:%d, reg_loop=0x%x \n",
			is_usb_chg_plugged_in(chip),
...
#if defined(CONFIG_LGE_PM) && !defined(CONFIG_MACH_LGE_FX3_VZW)
